...
from envs.sibrivalry.ant_maze import AntMazeEnv, AntMazeEnvFull, AntMazeFullDownscale
...
def make_env(args):
  ...
  elif 'antmazefulldownscale' in args.env.lower():
    env_fn = lambda: AntMazeEnvFullDownscale(variant='AntMazeFullDownscale-SR', eval=False)
    eval_env_fn = lambda: AntMazeEnvFullDownscale(variant='AntMazeFullDownscale-SR', eval=True)
